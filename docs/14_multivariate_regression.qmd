...
The second situation that I described is reflected by the `C` variable in @fig-waffle-abstract. `C` in this case is what we call a `Counfounding variable`. We've already talked about these in the context of visually examining conditional bivariate relationships [^14_multivariate_regression-1]. A counfounding variable, as is visually represented in @fig-waffle-abstract, explains both Waffle Houses and Divorce.
...
waffles %>% # grab my waffles dataset
    ...
    geom_point( # specifically, I am going to make a scatterplot
        aes(
            ...
            size = MedianAgeMarriage # change the size of the dots based on age of marraige variable
        ),
...
```{r}
#| lable: r-multiple-regression-with-table
