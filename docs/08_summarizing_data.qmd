...
box::use(
    ...
    #* Helpful functions for makeing tables of descriptive statistics
...
- With `dplyr`'s [@dplyr] `select()` function, I am going to `select()` the `gdppc` column
- With `dplyr`'s [@dplyr] `rename()` function, I am going to `rename()` the `gdppc` column to `GDP (per capita)`
