...
Or I could present the results as a plot with the `modelplot()` function from the `modelsummary` package [@modelsummary]. I personally find this output a bit prettier. It also let's me use functions from the `ggplot2` package [@ggplot2] like adding notes to the plot and changing the theme! 😎
