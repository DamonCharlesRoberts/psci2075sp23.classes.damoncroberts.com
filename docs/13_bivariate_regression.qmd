...
box::use(
    ...
    ggplot2 = ggplot2[ggplot, aes, geom_point, geom_smooth, theme_minimal, labs, geom_histogram]
)
# Load data
...
But this is a bit tricky and can be harder with other data. So let's have R apply a `line of best fit` that puts a straight line through the data points that `best` fit the data points.
...
We still see that we've got information on our $\alpha$, that it equals 9.460, and our $\beta$, that it equals 0.007. But what are those numbers in the parenthases underneath the what we sometimes call `point estimates` for $\alpha \text{and} \beta$? They are called `standard errors`.
...
model %>% # take the model object
    ggplot() + # make a plot with it
    geom_histogram( # specifically a histogram
        aes(x = residuals), # and put the residuals on the x-axis
